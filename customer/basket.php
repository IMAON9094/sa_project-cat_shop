<?php
        ...
        switch($_GET["action"]){
            case "add":
                if(!empty($_POST["quantity"])){
                    $select_add_query = "SELECT * FROM posts WHERE post_id='". $_GET["post_id"]. "'";
                    $result = mysqli_query($conn, $select_add_query);
                    while($row=mysqli_fetch_assoc($result)) {
                        $resultset[] = $row;
                    }
                    $productByCode = $resultset;
                    $itemArray = array($productByCode[0]["post_id"]=>(array('post_species'=>$productByCode[0]["post_species"],
                                                                            'post_id'=>$productByCode[0]["post_id"],
                                                                            'post_caatcolor'=>$productByCode[0]["post_catcolor"],
                                                                            'post_eyecolor'=>$productByCode[0]["post_eyecolor"],
                                                                            'post_catage'=>$productByCode[0]["post_catage"],
                                                                            'post_catsex'=>$productByCode[0]["post_catsex"],
                                                                            'post_catweight'=>$productByCode[0]["post_catweight"],
                                                                            'post_price'=>$productByCode[0]["post_price"],
                                                                            'post_image'=>$productByCode[0]["post_image"],
    ...
    if(isset($_POST['submitpayment'])){
        ...
        $insert_query_basket = "INSERT INTO `basketorder`(`id`, `basket_id`, `basket_totalprice`) VALUES ('','$bsk_id','$total_pc')";
